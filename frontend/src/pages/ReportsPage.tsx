...
const PostReviewModal = ({ postId, onClose }: { postId: string; onClose: () => void }) => {
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          {error && <div className="text-red-600 text-center py-8">Error loading post</div>}
...
const ReportsPage = () => {
  ...
  const [selectedReportPost, setSelectedReportPost] = useState<string | null>(null);
  ...
  if (Array.isArray(reportsData)) {
    ...
    reportsArray = Array.isArray(reportsData.reports) ? reportsData.reports : [];
    ...
    <div className="container mx-auto p-4">
      ...
      {activeTab === TABS.SUBMIT && (
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <p className="text-gray-600 mb-6">Report inappropriate user behavior or club content.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="mb-4">
              ...
              <select {...register('targetType')} id="targetType" className="w-full p-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                <option value="">Select report type...</option>
                <option value="user">User</option>
                <option value="club">Club</option>
              </select>
              {errors.targetType && <p className="text-red-500 text-sm mt-1">{errors.targetType.message}</p>}
          </div>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              <p className="text-sm text-gray-500 mt-1">Enter the student ID for user reports or club name for club reports</p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
              <p className="text-red-600">Error fetching reports: {error.message}</p>
            </div>
          )}
          ...
          {!isLoading && !error && reportsArray.length === 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
              ...
              {(statusFilter !== STATUS_FILTERS.ALL || typeFilter !== TYPE_FILTERS.ALL) && (
                <button
                  onClick={() => {
            ...
            <div className="space-y-4">
              {reportsArray.map((report: any) => (
                <div key={report._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="mb-3">
                    <p className="text-sm text-gray-600"><strong>Target:</strong> {report.targetId}</p>
                    <p className="text-sm text-gray-600 mt-1"><strong>Reason:</strong> {report.reason}</p>
      </div>
      ...
      {/* Post Review Modal */}
      {selectedReportPost && <PostReviewModal postId={selectedReportPost} onClose={() => setSelectedReportPost(null)} />}
    </div>
  );
};
