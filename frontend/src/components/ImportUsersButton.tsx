    ...
    onSuccess: (data) => {
      ...
      if (data.errors && data.errors.length > 0) {
        ...
        if (
          ...
          successfulCount === 0 &&
          existingUsersCount === data.errors.length
        ) {
          // All users already exist
          toast.error(
            `${existingUsersCount} ${
              existingUsersCount === 1 ? "user already" : "users already"
            } exist in the system`,
            { duration: 5000 }
          );
          ...
          if (newErrorsCount > 0) {
            message += `${newErrorsCount} ${
              newErrorsCount === 1 ? "error occurred" : "errors occurred"
            }.`;
          }
          toast(message, {
            duration: 6000,
